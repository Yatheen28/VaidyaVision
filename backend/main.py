...
Changes from previous versions:
  ...
  - Temperature scaling applied to raw model logits (calibrate.py output)
...
# ── Temperature scaling ──────────────────────────────────────────────────────
# Loaded from models/temperature.json (generated by calibrate.py).
# Falls back to T=1.5 if the file doesn't exist yet.
_TEMPERATURE = 1.5   # conservative fallback
...
def apply_temperature(raw_probs: np.ndarray, T: float) -> np.ndarray:
    """
    Divides log-probabilities by T before re-normalising.
    T > 1 spreads the distribution (less overconfident).
    T = 1 is identical to the original softmax output.
    """
...
def run_dl_model(img_rgb_uint8):
    ...
    Temperature scaling is applied before returning — all downstream code
    sees calibrated probabilities, not raw over-confident ones.
    """
    batch     = np.expand_dims(img_rgb_uint8.astype(np.float32), axis=0)
    raw_probs = dl_model.predict(batch, verbose=0)[0]
    cal_probs = apply_temperature(raw_probs, _TEMPERATURE)
    idx       = int(np.argmax(cal_probs))
    return (
        CLASS_NAMES[idx],
        float(cal_probs[idx]),
        {n: round(float(cal_probs[i]), 4) for i, n in enumerate(CLASS_NAMES)},
    )
